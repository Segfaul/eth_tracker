...
async def get_pair_data(trading_pair: str = "ETHUSDT") -> list:
    async with aiohttp.ClientSession() as session:
        async with session.get(f"https://api.binance.com/api/v1/klines?symbol={trading_pair.upper()}&interval=1m&limit=200", 
            proxy=PROXY,
...
async def get_prediction(dataset: list) -> str:
    ...
    df = pd.DataFrame(
        ...
        columns=[
            ...
            'Timestamp(close)', 'Quote', 'NumberOfTrades', 'TakerBuyBase',
    ...
    if 35 <= df.iloc[-1].RSI <= 43:
        ...
        rsi_indicator = False
    if 0.1 <= (100 - (df.iloc[-1].SMA9/df.iloc[-1].SMA21)*100) <= 0.2 and (100 - (df.iloc[-1].SMA9/df.iloc[-1].SMA200)*100) >= 0.3:
    ...
    if ((df.iloc[-1].Close >= df.iloc[-1].SMA21) and (arg) and (rsi_indicator == True) and (spread_between_sma == True)
        and (rsi_diff_l) and (df.iloc[-1].Close >= df.iloc[-1].PSAR)):
        return "buy"

    if ((df.iloc[-1].Close <= df.iloc[-1].SMA21) and (arg == False) and (rsi_indicator == False) and (rsi_diff_s)):
        return "sell"
    ...
    for user in (await user_service.get_ready_users):
        if stats['percent_change'] >= user['percent']:
            await notify_bot.bot.send_message(
                user['id'],
                f"{md.hbold('Trading Pair: ') + md.hlink(stats['trading_pair'], 'https://www.binance.com/en/futures/' + stats['trading_pair'])}"
...
async def tracking_loop(user_service, notify_bot, trading_pair: str = "ETHUSDT"):
    ...
    for i in range(1, 5):
        ...
        stats['percent_change'] = abs(
            round((float(stats['current']['price']) - float(stats['hour_ago']['price']))/float(stats['current']['price']), 2)
        ) * 100

        if stats['percent_change'] >= 1:

            if (not last_record) or (last_record['publication_date'] - stats['current']['time']) >= timedelta(minutes=60):
...
class TradingPairService():
    ...
    async def get_last_pair(cls, trading_pair: str):
        pair_obj = await TradingPair.filter(pair=trading_pair.lower()).order_by('-id').first().values()
        return pair_obj
